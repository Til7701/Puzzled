...
impl NavigationPresenter {
    ...
    pub fn show_puzzle_selection(&self, puzzle_collection: &PuzzleConfigCollection) {
        if let Some(presenters) = &self.presenters.borrow().as_ref() {
            presenters
                .puzzle_selection
                .show_collection(puzzle_collection);
            self.navigation_view.push_by_tag("puzzle-selection");
        }
    }

    pub fn show_puzzle_area(&self, puzzle_config: &PuzzleConfig) {
