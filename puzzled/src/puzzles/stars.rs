...
const STARS_LOOKUP: [StarsLookup; 5] = [
    ...
    // Easy
    StarsLookup {
        max_hints_used_for_stars: &[u32::MAX, 4, 0],
    },
    // Medium
    StarsLookup {
        max_hints_used_for_stars: &[u32::MAX, 3, 0],
    },
    // Hard
    StarsLookup {
        max_hints_used_for_stars: &[u32::MAX, 2, 0],
    },
    // Expert
    StarsLookup {
...
mod tests {
    ...
    #[test]
    fn test_calculate_stars_true_4_medium() {
        let stars = calculate_stars(true, Some(4), &Some(PuzzleDifficultyConfig::Medium));
        assert_eq!(stars.reached(), 1);
        assert_eq!(stars.total(), 3);
        assert_eq!(stars.max_hints_for_next_star(), Some(3));
    }
